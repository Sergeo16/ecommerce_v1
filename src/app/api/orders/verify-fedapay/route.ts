...
/**
 * Vérifie une transaction FedaPay et confirme la commande (côté serveur, anti-fraude).
 * POST body: { orderNumber: string, transactionId: number | string }
 */
export async function POST(request: NextRequest) {
  const userId = request.headers.get('x-user-id');
  let body: { orderNumber?: string; transactionId?: number | string };
  try {
  ...
  const transactionId =
    typeof body.transactionId === 'string' || typeof body.transactionId === 'number'
      ? body.transactionId
      : '';
  ...
  const verify = await verifyFedaPayTransaction(transactionId);
  if (!verify.success) {
    return NextResponse.json(
      { error: verify.message ?? 'Transaction FedaPay invalide ou échouée' },
      { status: 400 },
    );
  }
  ...
  const paidAmount =
    order.paymentMode === 'FULL_UPFRONT'
      ? Number(order.total)
      : Math.round((Number(order.total) * (order.advancePercent ?? 30)) / 100);
  ...
  if (paymentUserId) {
    await prisma.payment.create({
      data: {
        ...
        status: 'COMPLETED',
        externalId: String(verify.transactionId ?? transactionId),
        metadata: { gateway: 'FEDAPAY' },
      },
    });
  }
